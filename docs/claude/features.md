...
- **Service**: `backend/app/services/sync_service.py` (`run_sync()`, `get_or_create_schedule()`, `SyncCancelled`), `backend/app/services/scheduler.py` (`validate_cron()`, `reschedule()`, `start_scheduler()`, `get_scheduler()`)
