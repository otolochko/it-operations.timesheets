...
| `JIRA_WORKLOG_JQL` | Optional JQL filter clause (reserved for future use) |
...
- **Log text presentation**: `sync_runs.log_text` is maintained in the database table but excluded from the `SyncRunSummary` API schema; the frontend synthesizes a single-line summary rather than streaming stored execution logs.
